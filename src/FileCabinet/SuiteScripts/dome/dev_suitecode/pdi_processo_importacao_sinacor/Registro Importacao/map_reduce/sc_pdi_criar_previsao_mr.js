...
define([
    ...
    (runtime, format, record, utilsMd, cts) => {
        /**
        ...
        const getInputData = (inputContext) => {
            ...
            const {registroImportacao, isReprocessar} = JSON.parse(runtime.getCurrentScript().getParameter({name: "custscript_sc_preview_param"}));

            try {

                if(isReprocessar) utilsMd.deletePreviewTransaction(registroImportacao);
                ...
                const detailLines = utilsMd.getDetailImportRecord(registroImportacao);
        ...
        const map = (mapContext) => {
            ...
            const {registroImportacao} = JSON.parse(runtime.getCurrentScript().getParameter({name: "custscript_sc_preview_param"}));

            try {

                const lineObj = JSON.parse(mapContext.value);
                const allAccounts = utilsMd.getAllAccounts();
                const allAlocParam = utilsMd.getAllLoctionParam();
                const {subsidiary} = utilsMd.getImportRecordStatus(registroImportacao);
